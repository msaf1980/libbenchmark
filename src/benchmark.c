...
int64_t b_count(struct B *b) { return b->n; }
...
int b_prestart_timer(struct B *b) { 
    /* split with b_start_timer for allow run before sync start of threaded tests */
    if (!b->running) {
#ifdef BENCH_MEMSTAT
        get_alloc_stat(&b->allocstat); /* for reset stat */
#endif
    }
    return BENCH_SUCCESS;
}
...
#ifdef BENCH_MEMSTAT    
    result->alloc_bytes = (double) b->allocstat.bytes / b->n;
    result->alloc_count = (double) b->allocstat.count / b->n;
    result->alloc_failed = (double) b->allocstat.failed / b->n;
#endif
...
#ifdef BENCH_MEMSTAT
const char *table_mem_header_fmt =
    "\t%14s\t%8s\t%8s";
const char *table_mem_stats_fmt = "\t%14.2f\t%8.2f\t%8.2f";
#endif
...
#ifdef BENCH_MEMSTAT
    printf(table_mem_header_fmt, "alloc bytes/op", "alloc/op", "fail/op");
#endif
    printf("\n");
...
#ifdef BENCH_MEMSTAT
    printf(table_mem_stats_fmt, result->alloc_bytes, result->alloc_count, result->alloc_failed);
#endif    
