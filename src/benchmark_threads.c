...
#ifdef BENCH_MEMSTAT    
    result->alloc_bytes = 0;
    result->alloc_count = 0;
    result->alloc_failed = 0;
#endif    
    for (i = 0; i < threads; i++) {
        if (ts[i].b.n > 0) {
            int64_t j;
            ...
            n += ts[i].b.n;
#ifdef BENCH_MEMSTAT    
            result->alloc_bytes += (double) ts[i].b.allocstat.bytes / count;
            result->alloc_count += (double) ts[i].b.allocstat.count / count;
            result->alloc_failed += (double) ts[i].b.allocstat.failed / count;
#endif            
        }
    }
